...
export default function fetchFile(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const path0 = `${process.cwd()}/file/store/${key != null ? `${key}_` : ""}${
      quark_premium == true ? "1" : "0"
    }_0_${sha512().update(stringToHash).digest("hex")}.enc`;
    const path1 = `${process.cwd()}/file/store/${key != null ? `${key}_` : ""}${
      quark_premium == true ? "1" : "0"
    }_1_${sha512().update(stringToHash).digest("hex")}.enc`;
    ...
    if (path0 && existsSync(path0)) await sleep(10000);
            ...
            createDecipheriv(
              ...
              sha512()
                .update(
                  `${sha512()
                    .update(
                      `${guild_id}${channel_id}${attachment_id}${file_size}`
                    )
              ...
              sha512()
                .update(
                  `${sha512()
                    .update(`${guild_id}${channel_id}${attachment_id}`)
                    .digest("hex")}${file_size}`
                )
                .digest("hex")
                .slice(0, 16)
            )
    ...
    } else return reject(new Error("File does not exist"));
  });
}
