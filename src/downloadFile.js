...
export default function downloadFile(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (maxFileSize < file_size) return reject("File too big");
    ...
    const fileName0 = `${process.cwd()}/file/store/${
      key != null ? `${key}_` : ""
    }${quark_premium == true ? "1" : "0"}_0_${sha512()
      .update(stringToHash)
      .digest("hex")}.enc`;
    const fileName1 = `${process.cwd()}/file/store/${
      key != null ? `${key}_` : ""
    }${quark_premium == true ? "1" : "0"}_1_${sha512()
      .update(stringToHash)
      .digest("hex")}.enc`;
      ...
      .pipe(
        createCipheriv(
          ...
          sha512()
            .update(
              `${sha512()
                .update(`${guild_id}${channel_id}${attachment_id}${file_size}`)
